	    ...
	    try {
	        if (!oNavigator.ie) {
	            if (!!oNavigator.safari && oNavigator.version <= 5) {
	                ...
	                // if(BrowserDetect.browser === isMobile.any){
	                if (/Macintosh|Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) || 'ontouchend' in document) {
	...
	function html5Upload() {
	    ...
	    sUploadURL = 'file_uploader_html5_noti_modify.php'; //upload URL
	...
	function onAjaxError(e) {
	    alert("[가이드]사진 업로드할 서버URL셋팅이 필요합니다.-onAjaxError" + e._response.responseText);
	}
	...
	function callFileUploader() {
	    oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"), {
	        sUrl: location.href.replace(/\/[^\/]*$/, '') + '/file_uploader_noti_modify.php', //샘플 URL입니다.
